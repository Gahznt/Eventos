$(document).ready(function () {
    $(document).on('change', '#speaker_picture', function () {
        if (!this.files || !this.files[0] || !this.files[0].size) {
            return;
        }

        $('label[for="speaker_picture"]').text(this.files[0].name);
        $('#speakerPicturePath').attr('src', URL.createObjectURL(this.files[0]));
    });

    $(document).on('submit', 'form#speakerForm', function (e) {
        e.preventDefault();
